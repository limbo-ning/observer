...
func (p *Odor) uploadData(i *instruction.Instruction) error {
	defer func() {
		if err := recover(); err != nil {
			log.Println("error process upload odor data: ", err)
			return
		}
	}()
	...
	if err != nil {
		log.Println("process upload odor data error:", err)
		return err
	}
...
func (p *Odor) parseUploadData(i *instruction.Instruction) ([]data.IData, error) {
