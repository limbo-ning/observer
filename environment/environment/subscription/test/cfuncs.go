...
func hello(libpath string) error {
	return callLib(libpath, "hello_world", func(handle unsafe.Pointer) (err error) {

		if handle == nil {
			log.Println("handle nil")
		}
		log.Println(handle)

		ret, err := C.HelloCall(handle)
		if err != nil {
			return err
		}
...
func getSMSParam(siteID, libpath, subscriptionType string, isCease bool, e *Entity, s *Station, t time.Time, dataList []IData) (map[string]string, error) {
	...
	if err := callLib(libpath, "get_sms_param", func(handle unsafe.Pointer) (err error) {
		...
		if len(dataList) > 0 {
			...
			for i, d := range dataList {
				...
				case REAL_TIME:
					...
					cd.value = C.double(d.(IRealTime).GetRtd())
				case MINUTELY:
					cd.data_type = C.MINUTELY
					cd.value = C.double(d.(IInterval).GetAvg())
				case HOURLY:
		...
		log.Println("get sms param: ", t, t.Unix())
